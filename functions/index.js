...
// Helper to get API key for axios calls (legacy functions)
...
// =============================================================================
// PERSONALIZED AI RECOMMENDATIONS (v2 - Profile-Aware)
// Uses golfer profile settings to customize Claude API responses
// Patent Pending: Favorite Club Baseline methodology
// =============================================================================

// System prompt for personalized recommendations
const PERSONALIZED_SYSTEM_PROMPT = `You are a master club fitter with 30+ years of experience at top facilities like Club Champion and True Spec. You combine deep technical knowledge with practical wisdom about what actually helps golfers play better.

Your recommendations are:
- Evidence-based (grounded in launch monitor data and fitting principles)
- Practical (consider budget, availability, and diminishing returns)
- Honest (don't oversell small improvements)
- Personalized (adapted to this specific golfer's profile and goals)

IMPORTANT: Adapt your communication style and technical depth based on the golfer's stated preference.

When discussing equipment:
- Reference specific makes/models when relevant
- Explain WHY something would help, not just WHAT to buy
- Give price ranges and alternatives when recommending purchases
- Acknowledge when lessons/practice would help more than equipment

Format your response clearly with the most actionable insight first.`;

/**
 * Generate personalized AI recommendation using golfer profile
 * 
 * @param {string} userId - Client ID (null for self)
 * @param {string} analysisType - Type of analysis (bag_review, club_recommendation, etc.)
 * @param {string} userQuery - The specific question/context
 * @param {object} additionalContext - Optional extra data
 */
exports.generatePersonalizedRecommendation = onCall({
  timeoutSeconds: 60,
  memory: '512MiB'
}, async (request) => {
  const { userId, analysisType, userQuery, additionalContext } = request.data;
  ...
  // Security check: can only view own profile or managed clients
  if (targetUserId !== auth.uid) {
    const requestingUser = await db.doc(`users/${auth.uid}`).get();
    const targetUser = await db.doc(`users/${targetUserId}`).get();
    
    if (!requestingUser.exists || requestingUser.data().account_type !== 'professional') {
      throw new functions.https.HttpsError('permission-denied', 'Cannot generate recommendations for this user');
    }
    
    // Verify pro manages this client
    const proId = targetUser.data()?.pro_id || targetUser.data()?.proManagedInfo?.proUserId;
    if (proId !== auth.uid) {
      throw new functions.https.HttpsError('permission-denied', 'You do not manage this client');
    }
  }
  
  try {
    // Fetch profile
    const profileDoc = await db.doc(`users/${targetUserId}`).get();
    const profile = profileDoc.exists ? profileDoc.data() : {};
    
    // Fetch bag data
    const userRef = db.collection("users").doc(targetUserId);
    const bagSnapshot = await db.collection('clubs')
      .where('userId', '==', userRef)
      .get();
    ...
    if (!apiKey) {
      throw new functions.https.HttpsError('failed-precondition', 'AI service not configured. Set anthropic.api_key in Firebase config.');
    }
    ...
    await db.collection('aiRecommendations').add({
      ...
      analysisType,
      responseLength: recommendation.length,
      tokensUsed,
      model: 'claude-sonnet-4-20250514',
      timestamp: admin.firestore.FieldValue.serverTimestamp()
    });
    
    logger.info(`Personalized AI recommendation generated for ${targetUserId}. Tokens: ${tokensUsed}`);
    ...
    return {
      ...
      profileUsed: {
        ...
        primaryGoal: profile.primaryGoal || 'Not set',
        favoriteClubEnabled: profile.enableFavoriteClubGrading || false,
        handicap: profile.handicap || profile.Handicap || null,
        swingSpeed: profile.swingSpeed || null
      }
    };
    
  } catch (error) {
    logger.error('Personalized AI error:', error);
    
    if (error.code && error.code.startsWith('functions/')) {
      throw error;
    }
    
    throw new functions.https.HttpsError('internal', 'Failed to generate recommendation: ' + error.message);
  }
});

/**
 * Quick inline recommendation (lighter weight, ~300 tokens)
 * Use for inline tips on factor cards, quick suggestions, etc.
 */
exports.getQuickRecommendation = onCall(async (request) => {
  const { scenario, clubType, currentSpecs } = request.data;
  const auth = request.auth;
  ...
  const db = admin.firestore();
  const profileDoc = await db.doc(`users/${auth.uid}`).get();
  const profile = profileDoc.exists ? profileDoc.data() : {};
  
  const prompt = `
Golfer: ${profile.handicap || profile.Handicap || '?'} handicap, ${profile.swingSpeed || '?'} mph swing speed.
Goal: ${profile.primaryGoal || 'general improvement'}
Miss: ${profile.typicalMiss || 'not specified'}

Scenario: ${scenario}
Club Type: ${clubType}
Current Specs: ${JSON.stringify(currentSpecs)}

Give a brief (2-3 sentence) recommendation. Be specific and actionable.`;
  ...
  try {
    ...
    const response = await anthropic.messages.create({
      ...
      max_tokens: 300,
      temperature: 0.7,
      system: 'You are a golf club fitting expert. Give brief, specific recommendations.',
      messages: [{ role: 'user', content: prompt }]
    });
    
    return {
      success: true,
      recommendation: response.content[0]?.text || 'Unable to generate recommendation.'
    };
    
  } catch (error) {
    logger.error('Quick recommendation error:', error);
...
function buildPersonalizedPrompt(profile, bagData, analysisType, userQuery, additionalContext, swingProfile = null) {
  ...
  // ==========================================================================
  prompt += `## GOLFER PROFILE

**Physical & Swing Characteristics:**
- Handicap: ${profile.handicap || profile.Handicap || 'Unknown'}
- Driver Swing Speed: ${profile.swingSpeed ? profile.swingSpeed + ' mph' : 'Unknown'}
- Tempo: ${profile.tempo || 'Not specified'}
- Typical Miss: ${profile.typicalMiss || 'Not specified'}
- Ball Flight Preference: ${profile.ballFlight || 'Not specified'}
- Handedness: ${profile.handedness || 'Right'}
- Age: ${profile.age || 'Not specified'}
- Gender: ${profile.gender || 'Not specified'}

**Goals & Preferences:**
- Primary Goal: ${profile.primaryGoal || 'General improvement'}
- Communication Style: ${profile.communicationStyle || 'Balanced'}

**Baseline References:**
- Favorite Club: ${profile.favoriteClub || 'None set'}
...
`;
  ...
  const styleGuides = {
    'Simple Summary': `
COMMUNICATION STYLE: Simple Summary
- Use plain language, avoid jargon
- Focus on "what to do" not "why"
- One clear recommendation with 1-2 alternatives
- No technical specs unless directly relevant
- Keep response under 200 words
- Think: advice to a friend over beers`,

    'Balanced': `
COMMUNICATION STYLE: Balanced
- Mix of practical advice and key technical points
- Explain the "why" briefly when it helps understanding
- Include relevant specs (flex, weight) but don't overload
- 2-3 options with clear rationale
- Keep response 200-400 words
- Think: knowledgeable pro shop conversation`,

    'Full Technical Detail': `
COMMUNICATION STYLE: Full Technical Detail
- Include all relevant specifications and data
- Explain the physics/mechanics behind recommendations
- Reference launch monitor metrics and optimal ranges
- Discuss trade-offs in depth
- Can be 400-600 words if needed
- Think: Tour-level fitting consultation`
  };
  
  prompt += `\n---\n${styleGuides[profile.communicationStyle] || styleGuides['Balanced']}\n`;

  // ==========================================================================
  // GOAL-BASED PRIORITY GUIDANCE
  // ==========================================================================
...
Priority factors:
1. Ball speed optimization (smash factor, CG alignment)
2. Launch angle optimization (typically 10-15° for driver)
3. Spin reduction for woods (under 2500rpm ideal for most swing speeds)
4. Shaft weight reduction if swing speed allows
5. Loft optimization for carry vs. roll trade-off
Secondary: Maintain reasonable dispersion - distance means nothing OB`,

      'Better Accuracy': `
PRIMARY GOAL: Accuracy & Consistency
Priority factors:
1. Forgiveness (high MOI, larger sweet spots)
2. Dispersion tightening over maximum distance
3. Consistent gapping through the bag
4. Shot shape control (reduce curve on misses)
5. Confidence-inspiring designs at address
Secondary: Don't sacrifice more than 5-10 yards for accuracy gains`,

      'Lower Scores': `
PRIMARY GOAL: Lower Scores
Priority factors:
1. Scoring club optimization (wedges, short irons, putter)
2. Consistent distance gapping (no gaps > 15 yards)
3. Playability from all lies and conditions
4. Weakness identification and correction
5. Course management considerations
Think: What loses strokes? Fix that first.`,

      'More Forgiveness': `
PRIMARY GOAL: Maximum Forgiveness
Priority factors:
1. High MOI designs across the bag
2. Perimeter weighting and cavity backs
3. Wider soles for turf interaction
4. Game improvement features (offset, draw bias)
5. Lighter overall weight for easier swing
Accept trade-off: Some workability loss for consistency`,

      'Shot Shaping': `
PRIMARY GOAL: Shot Shaping Control
Priority factors:
1. Compact, workable club heads
2. Neutral face angles and CG positions
3. Responsive shafts that transmit feel
4. Consistent weighting for repeatable moves
5. Lower spin for controlling curve
Accept trade-off: Less forgiveness for more control`
    };
    
    if (goalGuides[profile.primaryGoal]) {
      prompt += `\n---\n${goalGuides[profile.primaryGoal]}\n`;
    }
  }

  // ==========================================================================
  // TYPICAL MISS CORRECTION GUIDANCE
  // ==========================================================================
  if (profile.typicalMiss) {
    const missGuides = {
      'Slice / Fade': `
TYPICAL MISS: Slice/Fade (ball curves right for RH golfer)
Fitting considerations:
- Draw-bias driver heads (heel weighting, closed face)
- Offset in irons can help square face
- Consider lighter shafts if over-swinging
- Shaft tip profile: softer tip can help close face
- Grip check: weak grip compounds slice
Note: Equipment helps but doesn't fix swing path`,

      'Hook / Draw': `
TYPICAL MISS: Hook/Draw (ball curves left for RH golfer)
Fitting considerations:
- Neutral or fade-bias heads
- Avoid offset in irons
- Stiffer tip profiles to prevent excessive closure
- Heavier shafts can calm aggressive transition
- Check for shafts that are too light/whippy
Note: Better players often hook - this isn't always "wrong"`,

      'High Miss': `
TYPICAL MISS: High ball flight (ballooning, excessive spin)
Fitting considerations:
- Lower lofts where appropriate
- Stiffer shafts to reduce dynamic loft
- Lower spin shaft profiles
- Heavier heads for lower launch
- Check for excessive attack angle with driver
Note: High can be good in soft conditions`,

      'Low Miss': `
TYPICAL MISS: Low ball flight (struggling to get airborne)
Fitting considerations:
- More loft (don't let ego drive decisions)
- Lighter, more flexible shafts
- Higher launching shaft profiles
- Shallower club head designs
- Check for steep attack angle with irons
Note: Low and penetrating is fine; low and no carry is the problem`,

      'Inconsistent': `
TYPICAL MISS: Inconsistent (no predictable pattern)
Fitting considerations:
- Maximum forgiveness is priority
- Consistent weighting through bag
- Avoid "tweener" specs - commit to a profile
- Shaft consistency (same weight/flex family)
- May indicate swing issue more than equipment
Note: Lessons may help more than new clubs here`
    };
    
    if (missGuides[profile.typicalMiss]) {
      prompt += `\n---\n${missGuides[profile.typicalMiss]}\n`;
    }
  }
  ...
  // FAVORITE CLUB BASELINE (PATENT PENDING)
  // ==========================================================================
  if (profile.enableFavoriteClubGrading && profile.favoriteClub) {
    const favClub = bagData.find(c => 
      c.clubType === profile.favoriteClub || 
      c.club_type === profile.favoriteClub
    );
    
    if (favClub) {
      const shaftWeight = favClub.shaft?.weight || favClub.shaftWeight || 'Unknown';
      const flex = favClub.shaft?.flex || favClub.shaft_flex || favClub.flex || 'Unknown';
      const length = favClub.length || 'Unknown';
      const make = favClub.brand || favClub.make || '';
      const model = favClub.model || '';
      
      prompt += `
---
FAVORITE CLUB BASELINE: ${profile.favoriteClub} [PATENT PENDING]

This golfer's best-performing club. Use its specs as the IDEAL baseline:
- Club: ${make} ${model}
- Shaft Weight: ${shaftWeight}${typeof shaftWeight === 'number' ? 'g' : ''}
- Shaft Flex: ${flex}
- Length: ${length}${typeof length === 'number' ? '"' : ''}

GRADING APPROACH:
Instead of grading against generic "optimal" charts, grade weight progression 
RELATIVE to this favorite club. This captures their ACTUAL tempo and timing 
preferences, not theoretical ideals.

Weight progression from favorite club:
- Longer clubs: subtract ~5g per club
- Shorter clubs: add ~5g per club
- Driver shaft should be ~40-50g lighter than favorite iron shaft
`;
    } else {
      prompt += `
---
FAVORITE CLUB BASELINE: ${profile.favoriteClub} (not found in bag)
Cannot apply baseline grading - club not present in current bag data.
Use standard fitting guidelines instead.
`;
    }
  }
  ...
  if (bagData && bagData.length > 0) {
    prompt += `\n---\n## CURRENT BAG COMPOSITION (${bagData.length} clubs)\n\n`;
    
    // Group by category
    const categories = {};
    bagData.forEach(club => {
      const cat = club.category || 'Other';
      if (!categories[cat]) categories[cat] = [];
      categories[cat].push(club);
    });
    
    for (const [category, clubs] of Object.entries(categories)) {
      prompt += `**${category}:**\n`;
      clubs.forEach(club => {
        const type = club.clubType || club.club_type || 'Unknown';
        const make = club.brand || club.make || '';
        const model = club.model || '';
        const shaft = club.shaft?.model || club.shaftModel || '';
        const flex = club.shaft?.flex || club.shaft_flex || club.flex || '';
        const loft = club.loft ? `${club.loft}°` : '';
        
        prompt += `- ${type}: ${make} ${model}`;
        if (shaft) prompt += ` / ${shaft}`;
        if (flex) prompt += ` (${flex})`;
        if (loft) prompt += ` ${loft}`;
        prompt += '\n';
      });
    ...
    prompt += '\n---\n## CURRENT BAG: No club data available.\n';
  }
  ...
  const analysisGuides = {
    'bag_review': `
ANALYSIS TYPE: Full Bag Review
Evaluate the complete bag composition for:
- Gapping consistency (no gaps > 15 yards)
- Weight progression through the bag
- Shaft consistency (flex, weight family)
- Club category balance (right mix of woods/hybrids/irons)
- Identify the 1-2 biggest improvement opportunities`,

    'club_recommendation': `
ANALYSIS TYPE: Specific Club Recommendation
Focus on:
- Why this specific club type needs attention
- 2-3 specific product recommendations with price ranges
- How the new club will integrate with existing bag
- Expected performance improvement`,

    'performance_test': `
ANALYSIS TYPE: Performance Test Analysis
Evaluate the test data for:
- Clear winner determination with reasoning
- Key metrics that drove the result
- What the data reveals about the golfer's swing
- Whether the difference is statistically meaningful
- Recommendation: keep, replace, or test more options`,

    'fitting_prep': `
ANALYSIS TYPE: Pre-Fitting Preparation
Help the golfer prepare for a professional fitting:
- What to expect in the fitting process
- Questions they should ask the fitter
- Areas of their game to focus on
- Budget considerations and value tiers
- Red flags to watch for`,

    'upgrade_priority': `
ANALYSIS TYPE: Upgrade Prioritization
Given their bag and goals, prioritize upgrades:
1. Highest impact change (best ROI)
2. Secondary opportunities
3. "Nice to have" improvements
4. What NOT to change (working well)
Include rough budget estimates for each tier`,

    'general': `
ANALYSIS TYPE: General Question
Answer the golfer's question with personalized context from their profile and bag data.`
  };
  
  prompt += `\n---\n${analysisGuides[analysisType] || analysisGuides['general']}\n`;
  ...
  if (additionalContext) {
    ...
    if (typeof additionalContext === 'string') {
      ...
      prompt += JSON.stringify(additionalContext, null, 2);
    }
    prompt += '\n';
  }
...
2. Match communication style to golfer preference
3. Prioritize based on their stated goal (if set)
4. Reference their specific data, not generic advice
5. If recommending products, give 2-3 options at different price points
6. Be honest about expected improvement (don't oversell)
7. Note when lessons/practice would help more than equipment
