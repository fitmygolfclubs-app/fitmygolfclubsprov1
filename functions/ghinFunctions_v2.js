...
/**
 * lookupGhinNumberInternal
 * 
 * Internal function for looking up a GHIN number (used by scheduled sync)
 * 
 * @param {string} ghinNumber - 7-8 digit GHIN number
 * @returns {object|null} { name, handicap, club, state, ghinNumber } or null
 */
async function lookupGhinNumberInternal(ghinNumber) {
  ...
  try {
    ...
    return {
      ...
      handicap: golfer.handicap_index || null,
      club: golfer.club_name || null,
      state: golfer.state || null,
      ghinNumber: cleanGhin
    };
  } catch (error) {
    console.error('GHIN lookup error:', error.message);
    return null;
  }
}

// Export for internal use by other functions
exports.lookupGhinNumberInternal = lookupGhinNumberInternal;
 ...
 * lookupGhinNumber
 * 
 * Simple GHIN lookup by number - used during client onboarding
 * Returns basic golfer info without requiring full account linking
 * 
 * @param {string} ghinNumber - 7-8 digit GHIN number
 * @returns {object|null} { name, handicap, club, state, ghinNumber } or null
 */
exports.lookupGhinNumber = functions.https.onCall(async (data, context) => {
  ...
  const result = await lookupGhinNumberInternal(ghinNumber);
  return result;
});
