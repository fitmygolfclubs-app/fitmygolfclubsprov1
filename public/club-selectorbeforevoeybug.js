...
const ClubSelector = (function() {
  ...
  function showModelSuggestions() {
    ...
    Object.entries(clubHeadSpecsCache).forEach(([docId, spec]) => {
      ...
      if (state.clubType) {
        ...
        // 1. Array: ['7i', '3i', 'Driver', ...]
        // 2. Object: {7i: {...}, Driver: {...}, ...}
        let hasClub = false;
        
        if (spec.clubs) {
          if (Array.isArray(spec.clubs)) {
            // Array format - case insensitive search
            hasClub = spec.clubs.some(c => 
              c.toLowerCase() === clubKey.toLowerCase()
            );
          } else if (typeof spec.clubs === 'object') {
            // Object format - check for key (case insensitive)
            const clubKeys = Object.keys(spec.clubs);
            hasClub = clubKeys.some(k => 
  ...
  function showStockShaft() {
    ...
    // Get stock shaft refs - check club-specific location first, then top level
    let stockShaftRefs = [];
    
    if (state.clubType && spec.clubs) {
      const clubKey = getClubKey(state.clubType);
      const clubData = spec.clubs[clubKey];
      if (clubData && clubData.stockShaftRefs) {
        stockShaftRefs = clubData.stockShaftRefs;
      }
    }
    
    // Fallback to top-level stockShaftRefs (legacy format)
    if (stockShaftRefs.length === 0 && spec.stockShaftRefs) {
      stockShaftRefs = spec.stockShaftRefs;
    }
  ...
  function open(options = {}) {
    ...
    state = {
      ...
      brand: options.prefill?.brand || null,
      model: options.prefill?.model || null,
      clubHeadSpecId: options.prefill?.clubHeadSpecId || null,
      shaftBrand: options.prefill?.shaftBrand || null,
      shaftModel: options.prefill?.shaftModel || null,
      shaftId: options.prefill?.shaftId || null,
      shaftSpecs: options.prefill?.shaftSpecs || {},
      isManualShaft: false,
      specs: options.prefill?.specs || {},
  ...
  function configureForMode(mode, options) {
    ...
    switch (mode) {
      ...
      case 'shaft-only':
        // Shaft selection only - skip club head selection
        document.getElementById('cs-title').textContent = options.title || 'Select Shaft';
        document.getElementById('cs-confirm').textContent = 'Select Shaft';
        ...
        // Store clubType if provided
        if (options.clubType) {
          state.clubType = options.clubType;
        }
        
        // Check if we have a clubHeadSpecId for stock shaft lookup
        if (options.clubHeadSpecId && options.clubType) {
          state.clubHeadSpecId = options.clubHeadSpecId;
          const spec = clubHeadSpecsCache[options.clubHeadSpecId];
          
          // Get club-specific stock shaft refs
          const clubKey = getClubKey(options.clubType);
          const clubData = spec?.clubs?.[clubKey];
          const hasStockShaft = clubData && clubData.stockShaftRefs && clubData.stockShaftRefs.length > 0;
          
          if (hasStockShaft) {
            // Has stock shaft - show stock/aftermarket buttons, default to stock
            document.getElementById('cs-shaft-stock-btn').classList.add('active');
            document.getElementById('cs-shaft-custom-btn').classList.remove('active');
            document.getElementById('cs-custom-shaft').style.display = 'none';
            showStockShaft();
          } else {
            // No stock shaft - default to aftermarket
            document.getElementById('cs-shaft-stock-btn').classList.remove('active');
            document.getElementById('cs-shaft-custom-btn').classList.add('active');
            document.getElementById('cs-stock-shaft-info').style.display = 'none';
            document.getElementById('cs-custom-shaft').style.display = 'block';
            setTimeout(() => {
              document.getElementById('cs-shaft-brand-input').focus();
          ...
          // No clubHeadSpecId - only show aftermarket option
          document.getElementById('cs-shaft-stock-btn').classList.remove('active');
          document.getElementById('cs-shaft-custom-btn').classList.add('active');
          document.getElementById('cs-stock-shaft-info').style.display = 'none';
          document.getElementById('cs-custom-shaft').style.display = 'block';
          
          // Focus shaft brand input
          setTimeout(() => {
            document.getElementById('cs-shaft-brand-input').focus();
          }, 100);
        }
      ...
      default:
        ...
        // Handle prefill data for edit mode
        if (options.prefill && options.prefill.brand) {
          // Prefill brand
          document.getElementById('cs-brand-input').value = options.prefill.brand;
          
          // Show model section
          document.getElementById('cs-model-section').style.display = 'block';
          
          if (options.prefill.model) {
            // Prefill model
            document.getElementById('cs-model-input').value = options.prefill.model;
            
            // Show shaft section with choice buttons
            document.getElementById('cs-shaft-section').style.display = 'block';
            
            if (options.prefill.shaftBrand) {
              // Show custom shaft entry since we have aftermarket shaft data
              document.getElementById('cs-shaft-stock-btn').classList.remove('active');
              document.getElementById('cs-shaft-custom-btn').classList.add('active');
              document.getElementById('cs-custom-shaft').style.display = 'block';
              
              // Prefill shaft brand
              document.getElementById('cs-shaft-brand-input').value = options.prefill.shaftBrand;
              document.getElementById('cs-shaft-model-section').style.display = 'block';
              
              if (options.prefill.shaftModel) {
                document.getElementById('cs-shaft-model-input').value = options.prefill.shaftModel;
                
                // Enable confirm button since we have all data
                document.getElementById('cs-confirm').disabled = false;
              }
            }
  ...
  function confirmSelection() {
    ...
    } else if (state.mode === 'shaft-only') {
      // Return shaft data only
      if (!state.shaftId && !state.shaftBrand) {
        console.warn('No shaft selected');
        return;
      }
