...
(function() {
    ...
    ClubSelector.open({
      ...
      onSelect: async (result) => {
        try {
          ...
          // Prompt to regrade
          promptRegradeAfterChange('Club added');
  ...
  /**
   * Prompt user to regrade after bag change
   */
  async function promptRegradeAfterChange(changeType) {
    // Use branded modal if available, otherwise fall back to confirm
    let shouldRegrade = false;
    
    if (typeof showConfirmModal === 'function') {
      shouldRegrade = await showConfirmModal(
        `${changeType}!`,
        `Would you like to regrade the bag now to see updated analysis?\n\n(Click Cancel to regrade later)`,
        'Yes, Regrade'
      );
    } else {
      shouldRegrade = confirm(
        `${changeType}!\n\nWould you like to regrade the bag now to see updated analysis?\n\n(Click Cancel to regrade later)`
      );
    }
    ...
    if (shouldRegrade) {
      // Trigger regrade
      if (typeof openRegradeModal === 'function') {
        openRegradeModal();
      } else if (typeof gradeBag === 'function') {
        gradeBag('fast');
      } else {
        console.warn('No regrade function available');
        showToast('Please click "Grade Bag" to update analysis');
      }
    }
  }
  ...
  window.editClubNew = function(clubId, clubData) {
    ...
    ClubSelector.open({
      ...
      prefill: {
        ...
        clubHeadSpecId: clubData.clubHeadSpecId,
        shaftBrand: clubData.shaft_brand,
        shaftModel: clubData.shaft_model,
        shaftId: clubData.shaftId,
        shaftSpecs: {
          weight: clubData.shaft_weight,
          flex: clubData.shaft_flex,
          kickPoint: clubData.shaft_kickpoint,
          torque: clubData.shaft_torque
        },
        specs: {
          loft: clubData.loft,
          lie: clubData.lie,
          length: clubData.length
        }
      },
      onSelect: async (result) => {
        try {
          ...
          // Prompt to regrade
          promptRegradeAfterChange('Club updated');
          
        } catch (error) {
          console.error('Error updating club:', error);
          showToast('Error updating club: ' + error.message, 'error');
        }
      }
    });
  };
  ...
  window.startBagOnboarding = function() {
    ...
    if (typeof BagOnboarding === 'undefined') {
      showToast('Bag onboarding component not loaded', 'error');
      return;
    }
