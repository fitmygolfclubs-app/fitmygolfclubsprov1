  ...
  function showModelSuggestions() {
    ...
    Object.entries(clubHeadSpecsCache).forEach(([docId, spec]) => {
      ...
      if (state.clubType) {
        ...
        // spec.clubs is an ARRAY like ['Driver', '3W', '7i', 'PW']
        // Check if array includes the club key (case-insensitive)
        if (spec.clubs && Array.isArray(spec.clubs)) {
          const hasClub = spec.clubs.some(c => 
            c.toLowerCase() === clubKey.toLowerCase()
          );
          if (!hasClub) return;
        } else {
          // No clubs array - skip this spec for typed searches
          return;
        }
      }
