...
const ClubSelector = (function() {
  ...
  function showModelSuggestions() {
    ...
    Object.entries(clubHeadSpecsCache).forEach(([docId, spec]) => {
      ...
      if (state.clubType) {
        ...
        // Check multiple possible database structures:
        // 1. spec.clubs object with clubKey (e.g., spec.clubs.driver)
        // 2. spec.type matches category (e.g., spec.type === "driver")
        // 3. spec.clubType matches directly
        const hasClubInSpecs = spec.clubs && spec.clubs[clubKey];
        const matchesType = spec.type && spec.type.toLowerCase() === clubKey;
        const matchesClubType = spec.clubType && spec.clubType.toLowerCase() === state.clubType.toLowerCase();
        
        if (!hasClubInSpecs && !matchesType && !matchesClubType) return;
      }
  ...
  function getClubKey(clubType) {
    ...
    // Convert "7-Iron" to "7i", "Driver" to "driver", etc.
    const t = clubType.toLowerCase();
    
    if (t === 'driver') return 'driver';
    if (t.includes('wood')) return t.replace('-wood', 'w').replace(' wood', 'w');
    if (t.includes('hybrid')) return t.replace('-hybrid', 'h').replace(' hybrid', 'h');
    if (t.includes('iron')) return t.replace('-iron', 'i').replace(' iron', 'i');
