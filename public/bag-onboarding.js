...
const BagOnboarding = (function() {
  ...
  function renderSameShaftQuestion() {
    ...
    const hasShaftFromSelector = state.setShaftSpec !== null;
    ...
    // Auto-select "Yes" if shaft was already captured AND not already answered
    if (hasShaftFromSelector && state.sameShaftAnswer === null) {
      state.sameShaftAnswer = true;
      // Update button state without full re-render to avoid loop
      const yesBtn = content.querySelector('.bo-question-btn');
